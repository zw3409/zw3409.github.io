...
We propose a density-dependent cost function for optimal transport. In this formulation, the cost between two points is defined through a minimal-action path whose Lagrangian depends on an underlying probability distribution. This makes two points close only when they can be connected by paths that avoid regions of small probability. The paper develops a numerical framework for the corresponding data-driven optimal transport problem, with applications to clustering and matching problems.
